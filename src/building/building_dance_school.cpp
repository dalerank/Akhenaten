...
void building_dancer_school::spawn_figure() {
    ...
    if (base.figure_spawn_delay > spawn_delay) {
        ...
        if (dest->id > 0) {
            create_figure_with_destination(FIGURE_DANCER, dest, (e_figure_action)ACTION_2_ENTERTAINER_GOING_TO_VENUE);
        } else {
            common_spawn_roamer(FIGURE_DANCER, current_params().min_houses_coverage, (e_figure_action)ACTION_0_ENTERTAINER_AT_SCHOOL_CREATED);
        }
    }
}
