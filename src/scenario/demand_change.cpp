    ...
    for (int i = 0; i < MAX_DEMAND_CHANGES; i++) {
        ...
        if (g_scenario.demand_changes[i].is_rise) {
            if (trade_route.increase_limit(resource) && g_empire.is_trade_route_open(route))
                events::emit(event_message{ true, "message_increased_trading", city_id, resource, SOURCE_LOCATION });

        } else {
            if (trade_route.decrease_limit(resource) && g_empire.is_trade_route_open(route)) {
                if (trade_route.limit(resource) > 0)
                    events::emit(event_message{ true, "message_decreased_trading", city_id, resource, SOURCE_LOCATION });
                else {
                    events::emit(event_message{ true, "message_trade_stopped", city_id, resource, SOURCE_LOCATION });
                }
            }
        }
    }
