...
struct file_dialog_save : public autoconfig_window_t<file_dialog_save> {
...
file_dialog_save g_file_dialog_save;

static void set_chosen_filename(const char* name) {
    auto& data = g_file_dialog_save;
...
static void draw_foreground(int) {
    ...
    if (data.message_not_exist_start_time
        ...
        int text_id = (data.type == FILE_TYPE_SCENARIO ? 3 : 0);
