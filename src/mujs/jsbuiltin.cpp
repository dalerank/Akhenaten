...
void jsB_props(js_State* J, const js_StringNode name, const char* string) {
    J->pushliteral(js_intern(string));
...
static void jsB_decodeURI(js_State* J) {
    Decode(J, js_strnode_cstr(js_tostring(J, 1)), URIRESERVED "#");
}

static void jsB_decodeURIComponent(js_State* J) {
    Decode(J, js_strnode_cstr(js_tostring(J, 1)), "");
}

static void jsB_encodeURI(js_State* J) {
    Encode(J, js_strnode_cstr(js_tostring(J, 1)), URIUNESCAPED URIRESERVED "#");
}

static void jsB_encodeURIComponent(js_State* J) {
    Encode(J, js_strnode_cstr(js_tostring(J, 1)), URIUNESCAPED);
}
