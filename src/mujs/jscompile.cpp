...
js_StringNode property_arguments = js_intern("arguments");
js_StringNode property_eval = js_intern("eval");
...
static int addstring(JF, const js_StringNode value) {
    int i;
    ...
    if (F->strlen >= F->strcap) {
        ...
        F->strtab = (js_StringNode*)js_realloc(J, F->strtab, F->strcap * sizeof *F->strtab);
    }
...
static void addlocal(JF, js_Ast* ident, int reuse) {
    js_StringNode name = ident->string;
    if (J->strict) {
        if (name == property_arguments) {
            jsC_error(J, ident, "redefining 'arguments' is not allowed in strict mode");
        }

        if (name == property_eval) {
    ...
    if (reuse || J->strict) {
        ...
        for (i = 0; i < F->varlen; ++i) {
            if (F->vartab[i] == name) {
                ...
                if (J->strict) {
                    jsC_error(J, ident, "duplicate formal parameter '%s'", js_strnode_cstr(name));
                }
            }
        }
    }
    ...
    if (F->varlen >= F->varcap) {
        ...
        F->vartab = (js_StringNode*)js_realloc(J, F->vartab, F->varcap * sizeof *F->vartab);
    }

    F->vartab[F->varlen++] = name;
}

static int findlocal(JF, const js_StringNode name) {
    int i;
...
static void emitstring(JF, int opcode, const char* str) {
    emit(J, F, opcode);
    emitraw(J, F, addstring(J, F, js_intern(str)));
}

static void emitstring(JF, int opcode, js_StringNode const& str) {
    emitstring(J, F, opcode, js_strnode_cstr(str));
}

static void emitlocal(JF, int oploc, int opvar, js_Ast* ident) {
    int i;
    if (J->strict && oploc == OP_SETLOCAL) {
        if (ident->string == property_arguments)
            jsC_error(J, ident, "'arguments' is read-only in strict mode");

        if (ident->string == property_eval)
...
static void checkdup(JF, js_Ast* list, js_Ast* end) {
    ...
    if (end->a->type == EXP_NUMBER)
        ...
        needle = js_strnode_cstr(end->a->string);
    ...
    while (list->a != end) {
        if (list->a->type == end->type) {
            ...
            if (prop->type == EXP_NUMBER)
                ...
                straw = js_strnode_cstr(prop->string);
...
static int has_modifier(js_AstModifier* modifiers, const js_StringNode key) {
    while (modifiers) {
...
static js_Ast* find_object_property(js_Ast* objlist, const js_StringNode name) {
    while (objlist) {
        js_Ast* prop = objlist->a;
        if (prop && prop->a) {
            js_Ast* propname = prop->a;
            if ((propname->type == AST_IDENTIFIER || propname->type == EXP_STRING) && propname->string == name) {
                return prop;
            }
        }
        objlist = objlist->b;
...
static js_Ast* new_str_ast_node(js_State* J, enum js_AstType type, js_StringNode s, int line) {
    js_Ast* node = new_ast_node(J, type, 0, 0, 0, 0, line);
    node->string = std::move(s);
    return node;
}

/* Helper to create a getter function that calls __property_getter if it exists */
static js_Ast* create_property_getter_ast(js_State* J, js_Ast* prop_node, js_StringNode property_name) {
    ...
    js_Ast* property_getter_name = new_str_ast_node(J, AST_IDENTIFIER, js_intern("__property_getter"), line);
    js_Ast* property_getter_member = new_ast_node(J, EXP_MEMBER, this_node, property_getter_name, 0, 0, line);

    /* "property_name" */
    js_Ast* prop_name_str = new_str_ast_node(J, EXP_STRING, std::move(property_name), line);
    ...
    js_Ast* cond_expr
      = new_ast_node(J, EXP_COND, property_getter_member, property_getter_call, undefined_node, 0, line);
...
static js_Ast* create_property_setter_ast(js_State* J, js_Ast* prop_node, js_StringNode property_name) {
    ...
    js_Ast* property_setter_name = new_str_ast_node(J, AST_IDENTIFIER, js_intern("__property_setter"), line);
    ...
    js_Ast* value_param = new_str_ast_node(J, AST_IDENTIFIER, js_intern("value"), line);

    /* this.__property_setter("property_name", value) */
    js_Ast* value_ident = new_str_ast_node(J, EXP_IDENTIFIER, js_intern("value"), line);
...
static void cobject(JF, js_Ast* list) {
    ...
    while (list) {
        ...
        if (kv->type == EXP_PROP_VAL && kv->modifiers && has_modifier(kv->modifiers, property_property)) {
            ...
            if (kv->b && kv->b->type == EXP_OBJECT) {
                ...
                if (kv->b->a) {
                    getter_prop = find_object_property(kv->b->a, property_get);
                    setter_prop = find_object_property(kv->b->a, property_set);
                }
...
static void ccall(JF, js_Ast* fun, js_Ast* args) {
    ...
    case EXP_MEMBER:
        ...
        break;
    case EXP_IDENTIFIER: {
        const char* funs = js_strnode_cstr(fun->string);
        if (fun->string == property_eval) {
            ceval(J, F, fun, args);
            return;
        }
        if (funs[0] == '_' && funs[1] == '_') {
            ...
            if (!ref) {
                ...
                if (local_idx < 0) {
                    ...
                    jsC_error(J, fun, "Function '%s' starting with __ is not registered as a global C-function", funs);
                }
                ...
                if (ref->value.type == JS_TOBJECT && ref->value.u.object) {
                    ...
                    if (obj->type == JS_CFUNCTION || obj->type == JS_CSCRIPT) {
                        ...
                        jsC_error(J, fun, "Function '%s' starting with __ must be registered as a global C-function",
                          funs);
                    }
                } else {
                    /* Not an object - must be C-function */
                    jsC_error(J, fun, "Function '%s' starting with __ must be registered as a global C-function", funs);
                }
            }
        }
    }
...
js_StringNode property_true = js_intern("true");
    ...
    case EXP_OBJECT:
        ...
        if (exp->modifiers) {
            ...
            while (mod) {
                ...
                emitstring(J, F, OP_STRING, mod->value ? mod->value : property_true);
                mod_count++;
                mod = mod->next;
            }
...
static int matchlabel(js_Ast* node, const js_StringNode label) {
...
static js_Ast* breaktarget(JF, js_Ast* node, const js_StringNode label) {
    while (node) {
        if (isfun(node->type))
            break;
        if (!label) {
...
static js_Ast* continuetarget(JF, js_Ast* node, const js_StringNode label) {
    while (node) {
        if (isfun(node->type))
            break;
        if (isloop(node->type)) {
...
static void ctrycatch(JF, js_Ast* trystm, js_Ast* catchvar, js_Ast* catchstm) {
    ...
    L1 = emitjump(J, F, OP_TRY);
    {
        /* if we get here, we have caught an exception in the try block */
        if (J->strict) {
            if (!strcmp(js_strnode_cstr(catchvar->string), "arguments"))
                jsC_error(J, catchvar, "redefining 'arguments' is not allowed in strict mode");
            if (!strcmp(js_strnode_cstr(catchvar->string), "eval"))
...
static void cstm(JF, js_Ast* stm) {
    ...
    case STM_BREAK:
        if (stm->a) {
            ...
            if (!target)
                jsC_error(J, stm, "break label '%s' not found", js_strnode_cstr(stm->a->string));
        } else {
    ...
    case STM_CONTINUE:
        if (stm->a) {
            ...
            if (!target)
                jsC_error(J, stm, "continue label '%s' not found", js_strnode_cstr(stm->a->string));
        } else {
...
static void analyze(JF, js_Ast* node) {
    ...
    if (node->type == EXP_IDENTIFIER) {
        if (!strcmp(js_strnode_cstr(node->string), "arguments")) {
            F->lightweight = 0;
            F->arguments = 1;
        } else if (!strcmp(js_strnode_cstr(node->string), "eval")) {
