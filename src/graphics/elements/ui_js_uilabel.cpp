...
static void jsB_UILabel_call(js_State *J) {
    js_typeerror(J, "UILabel is not callable");
}

static void jsB_UILabel_construct(js_State *J) {
    js_typeerror(J, "UILabel cannot be constructed");
}
...
ANK_REGISTER_UI_ELEMENT_PROTO(js_register_ui_element_label);

void js_register_ui_element_label(js_State *J) {
    ...
    js_newcconstructor(J, jsB_UILabel_call, jsB_UILabel_construct, ui::elabel::skind().c_str(), 0);
    js_defglobal(J, ui::elabel::skind().c_str(), JS_DONTENUM);

    js_ui_register_element_proto(ui::elabel::skind(), proto);
}
