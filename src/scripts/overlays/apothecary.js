...
function apothecary_building_tooltip(ev) {
    ...
    if (!house) {
        city.overlay_tooltip = "#apothecary_access_medium"
        return
    }

    var apothecary = house.apothecary
    if (apothecary <= 0) {
        city.overlay_tooltip = "#apothecary_access_none"
    } else if (apothecary >= 80) {
        city.overlay_tooltip = "#apothecary_access_high"
    } else if (apothecary < 20) {
        city.overlay_tooltip = "#apothecary_access_low"
    } else {
        city.overlay_tooltip = "#apothecary_access_medium"
    }
}
