...
#else
const std::string kGPUImageBaseBeautyFaceFragmentShaderString = R"(
    precision float; varying vec2 textureCoordinate; varying vec4 textureShift_1;
    varying vec4 textureShift_2;
    varying vec4 textureShift_3;
    varying vec4 textureShift_4;
...
bool BeautyFaceUnitFilter::Init() {
  ...
  gray_image_ = SourceImage::create(resPath + "/lookup_gray.png");
  original_image_ = SourceImage::create(resPath + "/lookup_origin.png");
  skin_image_ = SourceImage::create(resPath + "/lookup_skin.png");
  custom_image_ = SourceImage::create(resPath + "/lookup_light.png");
  return true;
}

bool BeautyFaceUnitFilter::proceed(bool updateSinks, int64_t frametime) {
  ...
  CHECK_GL(glBindTexture(GL_TEXTURE_2D,
                        _inputFramebuffers.at(2).frameBuffer->getTexture()));
  ...
  CHECK_GL(glBindTexture(GL_TEXTURE_2D, gray_image_->getFramebuffer()->getTexture()));
  _filterProgram->setUniformValue("lookUpGray", 5);

  CHECK_GL(glActiveTexture(GL_TEXTURE6));
  CHECK_GL(glBindTexture(GL_TEXTURE_2D, original_image_->getFramebuffer()->getTexture()));
  _filterProgram->setUniformValue("lookUpOrigin", 6);

  CHECK_GL(glActiveTexture(GL_TEXTURE7));
  CHECK_GL(glBindTexture(GL_TEXTURE_2D, skin_image_->getFramebuffer()->getTexture()));
  _filterProgram->setUniformValue("lookUpSkin", 7);

  CHECK_GL(glActiveTexture(GL_TEXTURE0));
  CHECK_GL(glBindTexture(GL_TEXTURE_2D, custom_image_->getFramebuffer()->getTexture()));
  ...
  _filterProgram->setUniformValue("widthOffset", width_offset);
  _filterProgram->setUniformValue("heightOffset", height_offset);

  // vertex position
  CHECK_GL(glVertexAttribPointer(_filterPositionAttribute, 2, GL_FLOAT, 0, 0,
                                imageVertices));

  _filterProgram->setUniformValue("sharpen", sharpen_factor_);
  _filterProgram->setUniformValue("blurAlpha", blur_alpha_);
  _filterProgram->setUniformValue("whiten", white_balance_);
