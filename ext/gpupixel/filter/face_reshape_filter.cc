...
bool FaceReshapeFilter::proceed(bool updateSinks, int64_t frametime) {
    float aspect = (float)_framebuffer->getWidth() / _framebuffer->getHeight();
    _filterProgram->setUniformValue("aspectRatio", aspect);

    _filterProgram->setUniformValue("thinFaceDelta", this->thin_face_delta_);

    _filterProgram->setUniformValue("bigEyeDelta", this->big_eye_delta_);

    _filterProgram->setUniformValue("hasFace", has_face_);
    if (has_face_) {
        _filterProgram->setUniformValue("facePoints", face_landmarks_.data(),
