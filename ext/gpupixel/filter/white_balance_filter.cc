...
const std::string kWhiteBalanceFragmentShaderString = R"(
    uniform sampler2D inputImageTexture; uniform float temperature;
    uniform float tint;
...
bool WhiteBalanceFilter::proceed(bool updateSinks, int64_t frametime) {
  _filterProgram->setUniformValue("temperature", temperature_);
  _filterProgram->setUniformValue("tint", tint_);
  return Filter::proceed(updateSinks, 0);
}
