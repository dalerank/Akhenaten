...
bool FaceMakeupFilter::Init() {
  ...
  // base render program
  filter_program2_ = GLProgram::createByShaderString(
  ...
  registerProperty("blend_level", 0.0f,
...
bool FaceMakeupFilter::proceed(bool updateSinks, int64_t frametime) {
  ...
  GPUPixelContext::getInstance()->setActiveShaderProgram(filter_program2_);
  ...
  CHECK_GL(glBindTexture(GL_TEXTURE_2D,
                        _inputFramebuffers.begin()->second.frameBuffer->getTexture()));
  ...
  CHECK_GL(glVertexAttribPointer(filter_tex_coord_attribute2_, 2, GL_FLOAT, 0, 0,
  ...
  std::shared_ptr<GPUPixelFramebuffer> fb = _inputFramebuffers.begin()->second.frameBuffer;
  ...
  CHECK_GL(glBindTexture(GL_TEXTURE_2D, image_texture_->getFramebuffer()->getTexture()));
  _filterProgram->setUniformValue("inputImageTexture2", 3);

  if (has_face_) {
    auto face_indexs = this->GetFaceIndexs();
    CHECK_GL(glDrawElements(GL_TRIANGLES, (GLsizei)face_indexs.size(), GL_UNSIGNED_INT,
                   face_indexs.data()));
  }
  _framebuffer->inactive();

  return Source::proceed(updateSinks, frametime);
}
