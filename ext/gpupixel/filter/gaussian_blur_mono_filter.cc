...
bool GaussianBlurMonoFilter::proceed(bool updateSinks, int64_t frametime) {
  ...
  if (rotationSwapsSize(inputRotation)) {
    if (type_ == HORIZONTAL) {
      ...
      _filterProgram->setUniformValue(
          "texelHeightOffset",
          (float)(vertical_texel_spacing_ / _framebuffer->getWidth()));
    } else {
      _filterProgram->setUniformValue(
          "texelWidthOffset",
          (float)(horizontal_texel_spacing_ / _framebuffer->getHeight()));
    ...
    if (type_ == HORIZONTAL) {
      _filterProgram->setUniformValue(
          "texelWidthOffset",
          (float)(horizontal_texel_spacing_ / _framebuffer->getWidth()));
      _filterProgram->setUniformValue("texelHeightOffset", (float)0.0);
    } else {
      _filterProgram->setUniformValue("texelWidthOffset", (float)0.0);
      _filterProgram->setUniformValue(
          "texelHeightOffset",
          (float)(vertical_texel_spacing_ / _framebuffer->getHeight()));
    }
  }
